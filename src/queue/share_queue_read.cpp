						 ...
						 int src_width, int src_height)
{
	...
	else
	{
		...
		if (x_ratio > y_ratio)
		{
			...
			dst_height = scale_info->dst_height;
			scale_info->dst_offset = ((scale_info->dst_width - dst_width) >> 2) << 2;
		}
		else if (x_ratio < y_ratio)
		{
			dst_height = min(scale_info->dst_height, src_height * x_ratio);
			dst_width = scale_info->dst_width;
			scale_info->dst_offset = (scale_info->dst_height - dst_height) / 2 * scale_info->dst_linesize[0];
		}
		else
		{
			dst_width = scale_info->dst_width;
							...
							uint8_t *dst, uint64_t *timestamp)
{
	...
	//source flip
	int32_t frame_linesize[4] = {0, 0, 0, 0};
	for (int i = 0; i < 4; i++)
	...
	if (v_flip)
	{
		data[0] += frame->linesize[0] * (frame->frame_height - 1);
		frame_linesize[0] = -frame_linesize[0];
	}
